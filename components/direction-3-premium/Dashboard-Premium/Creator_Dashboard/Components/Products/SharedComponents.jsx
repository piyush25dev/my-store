...
import { AlertCircle, ChevronDown, Loader, Trash2, Upload, X } from "lucide-react";
import {
  ADMIN_CONTROLLED_STATUSES,
  STATUS_OPTIONS,
  STATUS_STYLE,
  getToken,
} from "./utils";
...
// ─── StatusDropdown ───────────────────────────────────────────────────────────
// Creator-facing. Admin-controlled statuses show a read-only badge.
// Draft products can toggle between "draft" and "pending" (submit for review).
...
export function StatusDropdown({ productId, currentStatus, onStatusChange }) {
  ...
  const isAdminControlled = ADMIN_CONTROLLED_STATUSES.includes(currentStatus);
  ...
  // ── Read-only badge for admin-controlled statuses ──────────────────────────
  if (isAdminControlled) {
    return (
      <span
        title="This status is managed by admin"
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium cursor-default select-none
  ...
  // ── Editable dropdown for draft status ────────────────────────────────────
  // Creator can toggle between "draft" (save quietly) and "pending" (submit for review)
  const creatorOptions = [
    STATUS_OPTIONS.find((o) => o.value === "draft"),
    STATUS_OPTIONS.find((o) => o.value === "pending"),
  ];
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-20 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-[170px]">
          {creatorOptions.map((opt) => (
            ...
            >
              ...
              {opt.value === currentStatus && <span className="ml-auto text-slate-400">✓</span>}
            </button>
          ))}
        </div>
      )}
