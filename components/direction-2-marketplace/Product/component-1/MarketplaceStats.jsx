  ...
  return (
    <Card className="bg-white border-stone-200/60 shadow-sm">
      <CardHeader className="pb-2 pt-4 px-5">
        <h3 className="font-display text-stone-900 text-base">Marketplace Stats</h3>
      </CardHeader>
...
function CompactProductCard({ product }) {
  return (
    ...
    >
      ...
      <span className="font-display text-sm text-stone-900 shrink-0">₹{product.price.toLocaleString()}</span>
    </Link>
  );
}
...
function FullProductCard({ product }) {
  return (
    ...
    >
      <div className="flex gap-4 p-3 rounded-xl border border-stone-100 hover:border-rose-500/60 hover:bg-rose-100/20 transition-colors">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="font-display text-stone-900 text-sm mt-1.5">₹{product.price.toLocaleString()}</p>
        </div>
      </div>
    </Link>
  );
